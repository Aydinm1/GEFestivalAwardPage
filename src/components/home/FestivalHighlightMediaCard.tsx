    ...
    <>
      <div className="relative aspect-[16/9] overflow-hidden bg-black/20">
        ...
        <div className="absolute inset-x-0 top-0 flex items-center justify-between p-3">
          <span className="rounded-full border border-white/18 bg-black/30 px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.24em] text-white/92 backdrop-blur-sm">
            {mediaTypeLabel[item.type]}
          </span>
          {isActive ? (
            <span className="rounded-full border border-emerald-200/20 bg-emerald-300/18 px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.2em] text-emerald-50 backdrop-blur-sm">
              Active
            </span>
          ) : null}
        </div>
      ...
      <div className="flex flex-1 flex-col justify-between gap-4 p-5 text-left">
        <h3 className="font-headline text-[1.1rem] font-bold leading-tight text-white">
          {item.title}
        </h3>
        <div className="flex items-center justify-between gap-3 border-t border-white/10 pt-3">
          <span className="text-[11px] font-bold uppercase tracking-[0.24em] text-white/68 transition-colors group-hover:text-white/92">
            {item.type === 'video'
              ? isActive
                ? 'Watching'
                : 'Watch video'
          ...
          <span className="flex h-9 w-9 items-center justify-center rounded-full border border-white/14 bg-white/10 text-white/88 transition-all duration-300 group-hover:translate-x-0.5 group-hover:border-white/26 group-hover:bg-white/14">
            <svg
              aria-hidden="true"
              viewBox="0 0 20 20"
